...
- `convex/` - Convex backend: `schema.ts` (products/orders/subscribers), `products.ts` (public catalog queries), `seed.ts` (demo catalog seeder), `admin.ts` (password-guarded admin CRUD/orders/upload), `orders.ts` (`demoPay` — records paid orders until Razorpay keys arrive), `checkout.ts` + `internal/checkout.ts` + `http.ts` (Razorpay order creation, signature verification, `/razorpay-webhook`)
- `src/components/AdminPanel.tsx` + `src/lib/adminApi.ts` - client admin UI at the `#/admin` hash route (login uses the `ADMIN_PASSWORD` deployment env var, held in sessionStorage during the session)
